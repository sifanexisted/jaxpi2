...
def train_time_window(config, model, evaluator, logger, samplers, t_star, mesh, u_star, v_star, w_star, window_idx):
    ...
    for step in range(config.training.max_steps):
        ...
        if jax.process_index() == 0:
            if step % config.logging.log_every_steps == 0:
                ...
                log_dict = evaluator(model, model.state, loss_dict, batch, t_star[::4], mesh[::4], u_star[::4, ::4],
                                     v_star[::4, ::4], w_star[::4, ::4])
...
def train_and_evaluate(config, workdir):
    ...
    # Initialize model
    model = models.NavierStokes2D(config, lr, tx, arch, state, t_max=t1, nu=nu)

    # Initialize evaluator
    evaluator = models.NavierStokes2DEvaluator(config)
    ...
    for idx in range(config.training.num_time_windows):
        ...
        model = train_time_window(
            config, model, evaluator, logger, samplers, t_star, mesh, u_star, v_star, w_star, idx + 1)
