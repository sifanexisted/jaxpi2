...
def get_config():
    ...
    wandb.name = "plain_pseudo_time_fixed_batch"
    wandb.tag = None
    ...
    training.random_sampling = False
    ...
    pseudo_time.pts_weights = ml_collections.ConfigDict(
        {"ru": 5.0, "rv": 5.0, "rc": 5.0})
    pseudo_time.update_schedule = ml_collections.ConfigDict({
        "schedule": "constant",  # "exponential" or "constant", constant means update every fixed number of steps
        "start": 100,
        "every": 1000,  # used when schedule="constant"
        "first_every": 10,  # used when schedule="exponential"
        "max_every": 10000,
        "growth": 1.25,
    })
