...
class NavierStokes2D(ForwardIVP):
    def __init__(self, config, lr, tx, arch, state, t_max, nu):
        ...
        amplitude = config.problem.force_amplitude
        wavenumber = config.problem.force_wavenumber
        self.body_force_fn = lambda x, y: amplitude * jnp.sin(wavenumber * jnp.pi * y)

        # vmap functions over a spatial grid at a fixed time
        self.uvp0_pred_fn = vmap(self.neural_net, (None, None, 0, 0))
        self.w0_pred_fn = vmap(self.w_net, (None, None, 0, 0))

        # vmap functions over space and time
