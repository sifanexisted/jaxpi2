...
import models
from evaluators import GrayScottEvaluator
...
def train_and_evaluate(config):
    ...
    model = create_model(config, models.GrayScott, t_max=t1, b1=b1, b2=b2, c1=c1, c2=c2, eps1=eps1, eps2=eps2)
    evaluator = GrayScottEvaluator(config)
    ...
    def make_samplers(window_idx):
        ...
        if not config.training.random_sampling:
            # Fixed residual batch (mainly to validate pseudo-time stepping)
            samplers["res"] = itertools.repeat(next(iter(res_sampler)))
        return samplers
