...
def train_and_evaluate(config):
    ...
    model = models.Euler1D(config, lr, tx, arch, state, rho0, u0, p0, t_star, x_star, left_coords, right_coords)

    # Initialize evaluator
    evaluator = models.Euler1DEvaluator(config)
    ...
    for step in range(config.training.max_steps):
        ...
        if jax.process_index() == 0:
            if step % config.logging.log_every_steps == 0:
                ...
                log_dict = evaluator(model, model.state, loss_dict, batch, rho_ref, u_ref, p_ref)
